"""Benchmark GigaAM on the `test_dataset_v1` speech/text dataset."""
...
WORD_TO_NUM = {
    ...
    "тысяча четыреста двадцать": "1420", "тысячу": "1000", "две тысячи": "2000",
    "девяноста пяти": "95", "девяноста": "90", "пяти": "5", "восьми": "8",
    "двенадцати": "12", "пятнадцати": "15",
}


def load_dependencies() -> tuple[Any, Any, Any]:
    """Load runtime dependencies required by the benchmark.

    Args:
        None.

    Returns:
        tuple[Any, Any, Any]: `AutoModelForSpeechSeq2Seq`, `AutoProcessor`, `pipeline`, and `jiwer`.
    ...
    try:
        ...
        raise SystemExit("Install transformers stack: pip install transformers torchaudio sentencepiece") from exc
    try:
        import jiwer
    except ImportError as exc:
        raise SystemExit("Install jiwer: pip install jiwer") from exc
    return AutoModelForSpeechSeq2Seq, AutoProcessor, pipeline, jiwer
...
def get_abbrev_stats(norm_ref: str, norm_hyp: str) -> tuple[int, int]:
    ...
    Returns:
        tuple[int, int]: Number of found abbreviations and total targets.
...
def normalize_text(text: str) -> str:
    ...
    Args:
        text (str): Raw source text from the dataset or model output.

    Returns:
        str: Normalized text with aligned punctuation and number handling.
...
def compute_cer(reference: str, hypothesis: str) -> float:
    """Compute character error rate without external CER helpers.
...
def collect_dataset_items(dataset_dir: str) -> list[tuple[str, Path, str]]:
    ...
    Args:
        dataset_dir (str): Dataset root with `speech` and `text` folders.

    Returns:
        list[tuple[str, Path, str]]: Sample id, audio path, and reference text.

    Raises:
        FileNotFoundError: If the expected dataset structure is missing.
    """
...
def build_pipeline() -> Any:
    """Build the CPU inference pipeline for GigaAM.

    Args:
        None.

    Returns:
        Any: Configured Hugging Face ASR pipeline.

    Raises:
        SystemExit: If dependencies are missing.
    """
    AutoModelForSpeechSeq2Seq, AutoProcessor, pipeline, _ = load_dependencies()
...
def run_benchmark(dataset_dir: str, results_dir: str) -> None:
    """Run GigaAM benchmark on the `test_dataset_v1` dataset.

    Args:
        dataset_dir (str): Dataset root with `speech` and `text` folders.
        results_dir (str): Directory for the output JSON file.
    ...
    """
    _, _, _, jiwer = load_dependencies()
    asr_pipe = build_pipeline()
    ...
    print(f"Running GigaAM benchmark for {len(items)} files")
...
def main() -> None:
    ...
    """
    parser = argparse.ArgumentParser(description="Benchmark GigaAM on test_dataset_v1")
    parser.add_argument("--dataset_dir", required=True, help="Path to test_dataset_v1 root directory")
    parser.add_argument("--out_dir", default="gigaam_benchmark_results_test_dataset", help="Results directory")
